...
class OrderController {
    async create(req, res, next) {
        try {
            const {product_name, order_status, date, product_location, volume, weight, price, quantity_product, marketplace, userId} = req.body
            const order = await Orders.create({product_name, order_status, date, product_location, volume, weight, price, quantity_product, marketplace, userId})
    ...
    async updateOrder(req, res) {
        const { id, date, order_status, product_location, volume, weight, price, quantity_product, product_name, marketplace, userId} = req.body; // Получаем обновленные данные о заказе из тела запроса
